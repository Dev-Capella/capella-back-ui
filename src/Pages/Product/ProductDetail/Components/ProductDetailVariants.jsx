...
import ProductDetailImageSidebar from "./ProductDetailImageSidebar";
import ProductDetailVariantItemAttributeValue from "./ProductDetailVariantItemAttributeValue";
...
export default function ProductDetailVariants() {
  ...
  const mediasColumnBody = (data, options) => {
    if (data.galleries.length > 0 || data.files.length>0) {
      //const fileUrl = URL.createObjectURL(data.files[0]);
      const fileUrl = data.galleries.length>0 ?data.galleries[0].medias[0].absolutePath : data.files[0].url;
      return (
        <div className="relative ">
          {data.galleries.length + data.files.length  !== 1 && (
            <Badge
              className="absolute top-0 right-0"
              value={"+" + (data.galleries.length+data.files.length - 1 )}
            />
          )}
          <Image
  ...
  return (
    <div className="grid">
      ...
      >
        <ProductDetailVariantItemAttributeValue
